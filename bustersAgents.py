...
class GreedyBustersAgent(BustersAgent):
    ...
    def findclosest(self, positive, pacmanPosition):
        cpos = None
        dist = float('inf')
        for pos in positive:
            currdist = self.distancer.getDistance(pacmanPosition, pos)
            if currdist < dist:
                dist = currdist
                cpos = pos
    ...
    def chooseAction(self, gameState):
        ...
        for d in livingGhostPositionDistributions:
            if len(d) > 0:
                positive.append(d.argMax())

        pos = self.findclosest(positive, pacmanPosition)
        ...
        for action in legal: 
            currdist = self.distancer.getDistance(Actions.getSuccessor(pacmanPosition, action), pos)
