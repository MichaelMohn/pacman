...
class ExactInference(InferenceModule):
    ...
    def observeUpdate(self, observation, gameState):
        ...
        """
        "*** YOUR CODE HERE ***"
        poss = self.allPositions
        jp = self.getJailPosition()
        pp = gameState.getPacmanPosition()
        

        for pos in poss:
            self.beliefs[pos] *= self.getObservationProb(observation, pp, pos, jp)
    ...
    def elapseTime(self, gameState):
        ...
        "*** YOUR CODE HERE ***"
        poss = self.allPositions
        disc = DiscreteDistribution()

        for old in poss: 
            oldp = self.beliefs[old]

            for p, pr in self.getPositionDistribution(gameState, old).items():
                delta = oldp * pr
                disc[p] += delta
...
class ParticleFilter(InferenceModule):
    ...
    def observeUpdate(self, observation, gameState):
        ...
        for particle in self.particles:
            ...
            weights[particle] = weights.get(particle, 0) + prob
    ...
    def elapseTime(self, gameState):
        ...
        for oldPos in self.particles:
            ...
            newPos = newPosDistribution.sample()
...
class JointParticleFilter(ParticleFilter):
    ...
    def initializeUniformly(self, gameState):
        ...
        self.particles = (jointPositions * (self.numParticles // len(jointPositions) + 1))[:self.numParticles]
    ...
    def observeUpdate(self, observation, gameState):
        ...
        for particle in self.particles:
            ...
            for i, ghostPos in enumerate(particle):
                weight *= self.getObservationProb(observation[i], pacmanPos, 
                                                ghostPos, self.getJailPosition(i))
